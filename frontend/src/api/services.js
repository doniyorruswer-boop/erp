...
export const authApi = {
  ...
  logout: (body = {}) => api.post(API_ENDPOINTS.AUTH.LOGOUT, body),
...
export const dashboardApi = {
  getStats: (params = {}) => {
    if (typeof params === "string") {
      return api.get(`${API_ENDPOINTS.DASHBOARD.STATS}?type=${params}`);
    }
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.DASHBOARD.STATS}${query ? "?" + query : ""}`);
  },
  getWidgets: (role) => api.get(`${API_ENDPOINTS.DASHBOARD.WIDGETS}${role ? "?role=" + role : ""}`),
  getPaymentStats: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.DASHBOARD.PAYMENT_STATS}${query ? "?" + query : ""}`);
  },
};
...
export const studentsApi = {
  getAll: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.STUDENTS.BASE}${query ? "?" + query : ""}`);
  },
  getOne: (id) => api.get(`${API_ENDPOINTS.STUDENTS.BASE}/${id}`),
  create: (data) => api.post(API_ENDPOINTS.STUDENTS.BASE, data),
  update: (id, data) => api.put(`${API_ENDPOINTS.STUDENTS.BASE}/${id}`, data),
  delete: (id) => api.delete(`${API_ENDPOINTS.STUDENTS.BASE}/${id}`),
  restore: (id) => api.post(API_ENDPOINTS.STUDENTS.RESTORE(id)),
};

export const coursesApi = {
  getAll: () => api.get(API_ENDPOINTS.COURSES.BASE),
  getOne: (id) => api.get(`${API_ENDPOINTS.COURSES.BASE}/${id}`),
  create: (data) => api.post(API_ENDPOINTS.COURSES.BASE, data),
  update: (id, data) => api.put(`${API_ENDPOINTS.COURSES.BASE}/${id}`, data),
  delete: (id) => api.delete(`${API_ENDPOINTS.COURSES.BASE}/${id}`),
  restore: (id) => api.post(API_ENDPOINTS.COURSES.RESTORE(id)),
};

export const groupsApi = {
  getAll: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.GROUPS.BASE}${query ? "?" + query : ""}`);
  },
  getOne: (id) => api.get(`${API_ENDPOINTS.GROUPS.BASE}/${id}`),
  create: (data) => api.post(API_ENDPOINTS.GROUPS.BASE, data),
  update: (id, data) => api.put(`${API_ENDPOINTS.GROUPS.BASE}/${id}`, data),
  delete: (id) => api.delete(`${API_ENDPOINTS.GROUPS.BASE}/${id}`),
  restore: (id) => api.post(API_ENDPOINTS.GROUPS.RESTORE(id)),
  addStudent: (groupId, studentId) =>
    api.post(API_ENDPOINTS.GROUPS.STUDENTS(groupId), { studentId }),
  removeStudent: (groupId, studentId) =>
    api.delete(API_ENDPOINTS.GROUPS.STUDENT_DETAIL(groupId, studentId)),
  generateLessons: (groupId, options = {}) =>
    api.post(API_ENDPOINTS.GROUPS.GENERATE_LESSONS(groupId), options),
};

export const attendanceApi = {
  getAttendance: (groupId, date) =>
    api.get(`${API_ENDPOINTS.ATTENDANCE.BASE}?groupId=${groupId}&date=${date}`),
  getMonthlyAttendance: (groupId, month) =>
    api.get(`${API_ENDPOINTS.ATTENDANCE.MONTHLY}?groupId=${groupId}&month=${month}`),
  markAttendance: (groupId, date, records) =>
    api.post(API_ENDPOINTS.ATTENDANCE.BASE, { groupId, date, records }),
};

export const paymentsApi = {
  getAll: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.PAYMENTS.BASE}${query ? "?" + query : ""}`);
  },
  getOne: (id) => api.get(`${API_ENDPOINTS.PAYMENTS.BASE}/${id}`),
  getSummary: () => api.get(API_ENDPOINTS.PAYMENTS.SUMMARY),
  create: (data) => api.post(API_ENDPOINTS.PAYMENTS.BASE, data),
  update: (id, data) => api.put(`${API_ENDPOINTS.PAYMENTS.BASE}/${id}`, data),
  delete: (id) => api.delete(`${API_ENDPOINTS.PAYMENTS.BASE}/${id}`),
  void: (id, reason) => api.post(API_ENDPOINTS.PAYMENTS.VOID(id), { reason }),
  refund: (id, reason) => api.post(API_ENDPOINTS.PAYMENTS.REFUND(id), { reason }),
};

export const leadsApi = {
  getAll: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.LEADS.BASE}${query ? "?" + query : ""}`);
  },
  getKanban: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(`${API_ENDPOINTS.LEADS.KANBAN}${query ? "?" + query : ""}`);
  },
  getOne: (id) => api.get(`${API_ENDPOINTS.LEADS.BASE}/${id}`),
  create: (data) => api.post(API_ENDPOINTS.LEADS.BASE, data),
  update: (id, data) => api.put(`${API_ENDPOINTS.LEADS.BASE}/${id}`, data),
  moveToStage: (id, stageId) => api.post(`${API_ENDPOINTS.LEADS.BASE}/${id}/stage`, { stageId }),
  convert: (id, data = {}) => api.post(`${API_ENDPOINTS.LEADS.BASE}/${id}/convert`, data),
  delete: (id) => api.delete(`${API_ENDPOINTS.LEADS.BASE}/${id}`),
  restore: (id) => api.post(`${API_ENDPOINTS.LEADS.BASE}/${id}/restore`),
};
...
export const pipelinesApi = {
  ...
  reorderStages: (pipelineId, stageIds) =>
    api.post(`/crm/pipelines/${pipelineId}/stages/reorder`, { stageIds }),
};
...
export const branchesApi = {
  ...
  assignUser: (branchId, userId, isDefault = false) =>
    api.post(`/branches/${branchId}/users/${userId}`, { isDefault }),
...
export const scheduleApi = {
  ...
  cancel: (id, cancelReason) => api.post(`/schedules/${id}/cancel`, { cancelReason }),
  reschedule: (id, data) => api.post(`/schedules/${id}/reschedule`, data),
};
...
export const employeesApi = {
  ...
  getPayrollSummary: (period) =>
    api.get(`/employees/payroll/summary${period ? "?period=" + period : ""}`),
};
