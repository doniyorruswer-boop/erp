  ...
  <div class="notifications-page p-4 font-lexend">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
      ...
      <StatsCard
        ...
        :value="`${telegramCount} ta`"
        icon="solar:plain-bold"
        variant="purple"
        value-class="text-sky-500"
      />
    </div>
    ...
    <DataTable
      ...
      :show-index="true"
      :show-per-page="true"
      search-placeholder="Sarlavha, qabul qiluvchi yoki matn..."
      row-key="id"
    >
      ...
      <template #cell(title)="{ row }">
        ...
        <div class="text-xs text-gray-400 mt-0.5 line-clamp-1 max-w-md">
      ...
      <template #cell(status)="{ row }">
        <Badge :variant="row.status === 'FAILED' ? 'danger' : 'success'" :dot="true" size="sm">
          {{ row.status || "SENT" }}
        </Badge>
      </template>
      ...
      <template #actions="{ row }">
        <div class="flex items-center justify-end gap-2">
          ...
          <span v-else class="text-xs text-gray-400"> O'qilgan </span>
        </div>
      </template>
    </DataTable>
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-md max-w-lg w-full p-6 shadow-xl border dark:border-gray-700 font-lexend"
      >
        <div class="flex items-center justify-between pb-4 border-b dark:border-gray-700">
          ...
          <button class="text-gray-400 hover:text-gray-600" @click="showSendModal = false">
            <Icon icon="solar:close-circle-bold" class="w-6 h-6" />
          </button>
        </div>

        <form class="mt-4 space-y-4" @submit.prevent="sendNotification">
          <div>
  ...
  methods: {
    ...
    async sendNotification() {
      ...
      try {
        ...
        this.$toast.error(
          err.response?.data?.message || err.message || "Xabarnoma yuborishda xatolik"
        );
      } finally {
        this.sending = false;
      }
