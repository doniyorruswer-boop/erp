<template>
  <div class="student-profile-page p-4 font-lexend">
    ...
    <div v-else-if="!student" class="text-center py-20 bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-8">
      <Icon icon="solar:user-cross-bold" class="text-6xl text-gray-300 dark:text-gray-600 mx-auto mb-3" />
      <h3 class="text-lg font-bold text-gray-700 dark:text-gray-200">O'quvchi topilmadi</h3>
      <p class="text-sm text-gray-400 mt-1 mb-5">Bunday identifikatorli o'quvchi mavjud emas yoki o'chirilgan.</p>
      <router-link to="/students">
        <AppButton variant="primary" icon="solar:arrow-left-linear">O'quvchilar ro'yxatiga qaytish</AppButton>
      </router-link>
    </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5 shadow-sm">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          ...
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-tr from-primary to-indigo-400 text-white font-bold text-2xl flex items-center justify-center shadow-md flex-shrink-0">
            ...
            <div>
              <div class="flex items-center gap-2.5 flex-wrap">
                ...
                <Badge
                  :variant="student.status === 'ACTIVE' ? 'success' : student.status === 'FROZEN' ? 'warning' : 'danger'"
                  :dot="true"
                  size="sm"
                >
                  {{ student.status === 'ACTIVE' ? 'Faol o\'quvchi' : student.status === 'FROZEN' ? 'Muzlatilgan' : 'Nofaol' }}
                </Badge>
              </div>

              <div class="flex items-center gap-4 mt-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 flex-wrap">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <AppButton
              variant="outline"
              :icon="student.status === 'FROZEN' ? 'solar:play-circle-bold' : 'solar:pause-circle-bold'"
              size="md"
        ...
        <div class="space-y-5">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5 shadow-sm">
            <div class="flex items-center justify-between pb-3 border-b dark:border-gray-700">
              <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Hisob Balansi</span>
              <Icon icon="solar:wallet-2-bold" class="text-xl text-primary" />
            </div>

            <div class="mt-4">
              <div class="text-2xl font-black" :class="student.balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ formatUZS(student.balance) }}
              </div>
              <p class="text-xs text-gray-400 mt-1">
                {{ student.balance >= 0 ? 'To\'lovlar bo\'yicha qarzdorlik yo\'q' : 'Qarzdorlik mavjud! Iltimos, to\'lovni amalga oshiring.' }}
              </p>
            </div>
            ...
            <div class="mt-4 pt-4 border-t dark:border-gray-700 grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <span class="font-bold text-gray-800 dark:text-gray-200">{{ formatUZS(totalPaymentsSum) }}</span>
              </div>
              <div>
                <span class="text-gray-400 block">Oylik abonent:</span>
                <span class="font-bold text-gray-800 dark:text-gray-200">{{ formatUZS(currentMonthlyFee) }}</span>
              </div>
            </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5 shadow-sm">
            <div class="flex items-center justify-between pb-3 border-b dark:border-gray-700">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100">Biriktirilgan Guruhlar</h3>
              </div>
              <span class="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full text-gray-600 dark:text-gray-300 font-bold">
            ...
            <div class="mt-3 space-y-3">
              ...
              >
                <div class="flex items-center justify-between">
                  <span class="font-bold text-sm text-gray-800 dark:text-gray-100">{{ en.group?.name }}</span>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-2 space-y-1">
                  <div class="flex items-center gap-1.5">
                    ...
                    <span>{{ getDaysLabel(en.group?.days) }} ({{ en.group?.startTime }} - {{ en.group?.endTime }})</span>
                  </div>
                  <div class="flex items-center gap-1.5">
              ...
              <div v-if="!student.enrollments || student.enrollments.length === 0" class="text-center py-4 text-xs text-gray-400">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5 shadow-sm">
            <div class="flex items-center justify-between pb-3 border-b dark:border-gray-700">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100">Ota-ona ma'lumotlari</h3>
              </div>
            </div>
            ...
            <div class="mt-3 space-y-2 text-xs">
              <div class="flex justify-between py-1.5 border-b dark:border-gray-700/60">
                ...
                <span class="font-semibold text-gray-800 dark:text-gray-200">{{ student.parentName || '-' }}</span>
              </div>
              <div class="flex justify-between py-1.5 border-b dark:border-gray-700/60">
                <span class="text-gray-400">Telefon raqam:</span>
                <span class="font-semibold text-gray-800 dark:text-gray-200">{{ formatPhone(student.parentPhone) }}</span>
              </div>
        ...
        <div class="lg:col-span-2 space-y-5">
          ...
          <div v-if="activeTab === 'payments'" class="space-y-4">
            <div class="flex items-center justify-between bg-white dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
              <div>
                <h3 class="font-bold text-sm text-gray-800 dark:text-gray-100">Barcha To'lovlar Tarixi</h3>
                <p class="text-xs text-gray-400">Ushbu o'quvchi tomonidan amalga oshirilgan to'lovlar ro'yxati</p>
              </div>
              <AppButton
                variant="success"
                size="sm"
          ...
          <!-- TAB 4: Exams & Grades -->
          <div v-if="activeTab === 'exams'" class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5">
            <div class="flex items-center justify-between pb-4 mb-4 border-b dark:border-gray-700">
              <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-base text-gray-800 dark:text-gray-100">Imtihon va Test Natijalari</h3>
              </div>
            </div>
              ...
              >
                ...
                <div class="flex items-center gap-3">
                  <div class="text-right">
                    <span class="text-lg font-black" :class="exam.score >= 70 ? 'text-green-600' : 'text-amber-600'">
          ...
          <div v-if="activeTab === 'notes'" class="space-y-5">
            <!-- Add quick note -->
            <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4 shadow-sm">
              <h4 class="text-sm font-bold text-gray-800 dark:text-gray-100 mb-2">Ichki izoh / Eslatma qo'shish</h4>
              <div class="flex gap-2">
                <input
                  ...
                  @keyup.enter="addNote"
                />
                <AppButton variant="primary" icon="solar:plain-bold" @click="addNote">Qo'shish</AppButton>
              </div>
            </div>

            <!-- Notes List -->
            <div class="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-5">
              <h4 class="text-sm font-bold text-gray-800 dark:text-gray-100 mb-4">Tarix va Eslatmalar</h4>
              <div class="space-y-3">
                <div
                ...
                >
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between font-semibold text-gray-800 dark:text-gray-200">
    ...
    >
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6 border dark:border-gray-700">
        <div class="flex items-center justify-between pb-4 border-b dark:border-gray-700">
          <div class="flex items-center gap-2 text-green-600">
            ...
            <h3 class="text-base font-bold text-gray-800 dark:text-gray-100">To'lov Qabul Qilish</h3>
          </div>
        ...
        <form @submit.prevent="submitPayment" class="space-y-3.5 mt-4 text-sm">
          <div class="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg flex items-center justify-between text-xs">
            <span class="text-gray-400">O'quvchi:</span>
            <span class="font-bold text-gray-800 dark:text-gray-100">{{ student?.firstName }} {{ student?.lastName }}</span>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      newNoteText: "",
      notesList: [
        { author: "Admin", time: "Bugun 14:20", text: "Ota-onasi bilan dars jadvali bo'yicha bog'lanildi." },
        { author: "Menejer", time: "Kechagi kun", text: "Sinov darsiga qatnashdi va guruhga qabul qilindi." },
      ],
      paymentForm: {
        amount: 500000,
        method: "CASH",
  ...
  computed: {
    ...
    generalInfoItems() {
      ...
      return [
        { label: "Telefon Raqami", value: formatPhone(this.student.phone), icon: "solar:phone-calling-linear" },
        { label: "Yashash Manzili", value: this.student.address || "Manzil kiritilmagan", icon: "solar:map-point-linear" },
        { label: "Jinsi", value: this.student.gender === "FEMALE" ? "Ayol" : "Erkak", icon: "solar:users-group-rounded-linear" },
        { label: "Tug'ilgan Sana", value: formatDate(this.student.birthDate) || "Kiritilmagan", icon: "solar:calendar-date-linear" },
        { label: "Pasport / Metrika", value: this.student.passport || "AA 1234567", icon: "solar:card-2-linear" },
        { label: "PINFL (JShShIR)", value: this.student.pinfl || "30102030405060", icon: "solar:code-file-linear" },
      ];
    },
    academicInfoItems() {
      if (!this.student) return [];
      return [
        { label: "Qabul Sanasi", value: formatDate(this.student.createdAt), icon: "solar:calendar-linear" },
        ...
        { label: "Shartnoma Raqami", value: `#EDU-${this.student.id?.slice(0, 6)?.toUpperCase() || '2026'}`, icon: "solar:document-text-linear" },
        { label: "Qulay Dars Vaqti", value: "Tushdan keyin (14:00 - 18:00)", icon: "solar:clock-circle-linear" },
        { label: "Status", value: this.student.status === "ACTIVE" ? "Faol" : "Muzlatilgan", icon: "solar:check-circle-linear" },
      ];
    },
    totalPaymentsSum() {
  ...
  methods: {
    ...
    getMethodLabel(method) {
      const map = { CASH: "Naqd", CARD: "Plastik", PAYME: "Payme", CLICK: "Click", UZUM: "Uzum Pay" };
