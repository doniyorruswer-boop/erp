<template>
  <div class="audit-logs-page p-4 font-lexend space-y-5">
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200">
          Tizim Xavfsizlik Jurnali (Audit Log)
        </h1>
    ...
    <DataTable
      ...
      :show-index="true"
      :show-per-page="true"
      search-placeholder="Modul, ID yoki harakat..."
      row-key="id"
    >
      ...
      <template #cell(user)="{ row }">
        <div v-if="row.user">
          ...
          <div class="text-[11px] text-gray-400 font-mono">
            {{ row.user.phone || row.user.role }}
          </div>
        </div>
        <div v-else class="text-xs text-gray-400 italic">Tizim (Avtomatik)</div>
      </template>
      ...
      <template #cell(date)="{ row }">
        ...
        <div v-if="row.ip" class="text-[10px] text-gray-400 font-mono">IP: {{ row.ip }}</div>
      </template>
    ...
    <Vmodal
      ...
      :subtitle="
        selectedLog
          ? `${selectedLog.action} - ${selectedLog.entityType} (${formatDateTime(selectedLog.createdAt)})`
          : ''
      "
      width="max-w-xl"
      :hide-button="true"
    ...
    >
      <div v-if="selectedLog" class="space-y-4 text-xs">
        <div
          class="grid grid-cols-2 gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-md border dark:border-gray-700"
        >
          <div>
            ...
            <div class="font-semibold text-gray-800 dark:text-gray-200 mt-0.5">
              {{
                selectedLog.user
                  ? `${selectedLog.user.firstName} ${selectedLog.user.lastName} (${selectedLog.user.role})`
                  : "Tizim"
              }}
            </div>
          </div>
          <div>
            ...
            <div
              class="font-mono text-gray-800 dark:text-gray-200 mt-0.5 truncate"
        ...
        <div v-if="selectedLog.before">
          ...
          <pre
            class="p-3 bg-gray-900 text-emerald-400 rounded-md font-mono text-[11px] overflow-x-auto max-h-44"
        ...
        <div v-if="selectedLog.after">
          ...
          <pre
            class="p-3 bg-gray-900 text-sky-400 rounded-md font-mono text-[11px] overflow-x-auto max-h-44"
