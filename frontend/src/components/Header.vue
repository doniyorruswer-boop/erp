<template>
  ...
  <header
    class="sticky top-0 z-30 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md px-3 sm:px-4 py-2 border-b dark:border-gray-700 shadow-xs font-lexend transition-colors"
  >
    <div class="wrap-header flex items-center justify-between gap-2 sm:gap-4 flex-nowrap w-full">
      ...
      <div class="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        <div v-click-outside="closeBranchMenu" class="relative">
          <button
          ...
          >
            <Icon
              ...
              class="text-primary text-base sm:text-lg shrink-0"
            />
            <span class="max-w-[120px] sm:max-w-[200px] truncate font-medium">{{
              tenantStore.orgName
            }}</span>
            <Icon
              icon="solar:alt-arrow-down-linear"
              class="text-gray-400 text-xs sm:text-sm ml-0.5 shrink-0"
            />
          </button>
          ...
          <Transition name="fade">
            ...
            >
              <!-- Header -->
              <div
                class="px-4 py-3 bg-gray-50/90 dark:bg-gray-700/50 flex items-center justify-between"
              >
                <span
                  class="font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider"
                  >Muassasalar & Filiallar</span
                >
                <span
                  class="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-primary/10 text-primary"
                >
              ...
              <div class="py-1 max-h-80 overflow-y-auto divide-y dark:divide-gray-700/50">
                <button
                  ...
                  :class="[
                    ...
                    tenantStore.organization?.id === org.id
        ...
        <!-- Notifications -->
        <div v-click-outside="closeNotif" class="relative">
          <button
            class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 relative p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/60 transition cursor-pointer"
          ...
          >
            <span
              class="bg-red-500 rounded-full w-2 h-2 absolute top-1.5 right-1.5 ring-2 ring-white dark:ring-gray-800"
            ></span>
          ...
          <Transition name="fade">
            ...
            >
              <!-- Top Header -->
              <div
                class="flex justify-between items-center px-4 py-3.5 bg-slate-50/70 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-800"
              >
                <div class="flex items-center gap-2">
                  <h2 class="font-bold text-sm text-slate-800 dark:text-gray-100">
                    Bildirishnomalar
                  </h2>
                  <span
              ...
              <div
                class="divide-y divide-slate-100 dark:divide-gray-800/70 max-h-88 overflow-y-auto"
              >
                <div v-if="notifList.length === 0" class="py-10 text-center text-slate-400 text-xs">
                  <Icon
                    icon="solar:bell-linear"
                    class="text-3xl mx-auto mb-2 text-slate-300 dark:text-gray-600"
                  />
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <h3
                      class="font-semibold text-xs text-slate-800 dark:text-gray-100 leading-tight"
                    >
                      {{ item.name }}
                    </h3>
              ...
              <div
                class="p-2.5 bg-slate-50/70 dark:bg-gray-800/50 border-t border-slate-100 dark:border-gray-800 text-center"
              >
                <RouterLink
                  to="/notifications"
        ...
        <div v-click-outside="closeMenu" class="relative">
          ...
          >
            <div
              class="w-9 h-9 rounded-full bg-primary text-white font-bold flex items-center justify-center text-xs shadow-xs ring-2 ring-primary/20"
            >
              AD
            </div>
            <div class="text-left hidden sm:block">
              <h2 class="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-tight">
                Administrator
              </h2>
            ...
            >
              ...
              <div class="px-4 py-3">
                ...
                <p class="text-gray-400 text-xs truncate mt-0.5">
                  {{ authStore.currentUser?.email || $brand.adminEmail }}
                </p>
              </div>
...
import { loadDarkMode, setDarkMode } from "@/helper/theme";
        ...
        if (res && (res.data || Array.isArray(res))) {
          ...
          if (list.length > 0) {
            this.notifList = list.map((n) => ({
              ...
              hours: new Date(n.createdAt).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              }),
              isRead: n.isRead,
