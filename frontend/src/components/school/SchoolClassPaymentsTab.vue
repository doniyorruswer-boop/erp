<template>
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
      <div
        class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 space-y-1.5 shadow-xs"
      >
        <span class="text-gray-500 dark:text-gray-400 font-semibold text-sm"
          >Umumiy shartnoma rejasi:</span
        >
        <div class="text-2xl font-extrabold text-gray-900 dark:text-white">
          {{ formatMoneyUZS(classInfo.plan || 1499400000) }}
        </div>
        <span class="text-xs text-gray-400"
          >{{ students.length }} ta o'quvchi bo'yicha yillik reja</span
        >
      </div>
    ...
    <div
      class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-xs"
    >
      <div
        class="p-5 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between"
      >
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          <thead
            class="bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 font-bold uppercase text-xs sm:text-sm"
          >
            <tr>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            >
              ...
              <td class="py-3.5 px-4 font-semibold text-gray-800 dark:text-gray-200">
                {{ formatMoneyUZS(st.contractPlan || 53500000) }}
              </td>
              <td class="py-3.5 px-4 font-bold text-emerald-600 dark:text-emerald-400">
                {{ formatMoneyUZS(st.contractPaid || 26750000) }}
              </td>
              <td
                class="py-3.5 px-4 font-bold"
                :class="
                  (st.contractPlan || 53500000) - (st.contractPaid || 26750000) > 0
                    ? 'text-rose-500'
                    : 'text-emerald-500'
                "
              >
                {{
                  formatMoneyUZS(
                    Math.max(0, (st.contractPlan || 53500000) - (st.contractPaid || 26750000))
                  )
                }}
              </td>
