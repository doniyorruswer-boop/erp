<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-5 border border-gray-200 dark:border-gray-700 shadow-2xs space-y-4 font-lexend flex flex-col justify-between">
    <div>
      <!-- Header -->
      ...
      <div v-if="debtorsList.length === 0" class="py-8 text-center text-gray-400 dark:text-gray-500 text-xs">
        Hozirda qarzdor o'quvchilar mavjud emas
      </div>
      <div v-else class="space-y-3 pt-3">
        <div
          v-for="(d, idx) in debtorsList"
          :key="idx"
          class="group space-y-1"
        >
          <div class="flex items-center justify-between text-sm">
            <div class="min-w-0 pr-2">
              <span class="font-semibold text-gray-800 dark:text-gray-200 block truncate group-hover:text-rose-600 transition">
                {{ d.name }}
              </span>
          ...
          <div class="h-2 w-full bg-gray-100 dark:bg-gray-700/60 rounded-full overflow-hidden">
            <div
              ...
              :style="{ width: `${Math.min(100, Math.max(8, Math.round((d.amount / maxDebt) * 100)))}%` }"
            ></div>
          </div>
        </div>
      </div>
    ...
    <div class="pt-3 border-t border-gray-100 dark:border-gray-700/60 flex items-center justify-between text-xs sm:text-sm">
...
export default {
  ...
  computed: {
    debtorsList() {
      return (this.debtors && this.debtors.length > 0)
        ? this.debtors.slice(0, 5)
        : [];
    },
    maxDebt() {
