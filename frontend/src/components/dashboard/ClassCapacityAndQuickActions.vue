    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center pt-1">
      ...
      <div class="lg:col-span-7 flex items-center justify-between">
        ...
        >
          {{
            isKindergarten
              ? "GURUHLAR BO'YICHA BOLALAR VA SIG'IM"
              : "SINFLAR BO'YICHA O'QUVCHILAR SONI & SIG'IMI"
          }}
        </h3>
        <RouterLink
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
      ...
      >
        <div>
          <!-- Summary Header Bar -->
          <div
            class="flex items-center justify-between flex-wrap gap-2 pb-3 mb-3 border-b dark:border-gray-700/60 text-xs"
          >
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-xs text-left border-collapse">
              <thead>
                <tr
                  class="bg-gray-50/80 dark:bg-gray-700/60 text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider text-[10px] border-y dark:border-gray-700/60"
                >
                  ...
                  <th class="py-2.5 px-3 text-center">
                    {{ isKindergarten ? "BOLALAR" : "O'QUVCHILAR" }}
                  </th>
                ...
                >
                  ...
                  <td class="py-2.5 px-3 text-center whitespace-nowrap">
                    <span class="font-bold text-gray-800 dark:text-gray-100"
                      >{{ item.studentsCount }} ta</span
                    >
                    <span class="text-[10px] text-gray-400 block font-normal">
                      {{ item.boys || Math.round(item.studentsCount * 0.52) }} o'g'il,
                      {{ item.girls || Math.round(item.studentsCount * 0.48) }} qiz
                    </span>
                  </td>
                  ...
                  <td class="py-2.5 px-3 min-w-[130px]">
                    <div
                      class="flex items-center justify-between text-[10px] mb-1 font-semibold text-gray-600 dark:text-gray-300"
                    >
                      <span>{{ item.studentsCount }}/{{ item.capacity }} o'rin</span>
                      <span
                        :class="
                          item.studentsCount >= item.capacity ? 'text-rose-500' : 'text-emerald-500'
                        "
                      >
                        {{
                          item.studentsCount >= item.capacity
                            ? "To'lgan"
                            : `${item.capacity - item.studentsCount} ta bo'sh`
                        }}
                      </span>
                    </div>
                    <div
                    ...
                    >
                      <div
                        ...
                        :style="{
                          width: `${Math.min(100, Math.round((item.studentsCount / item.capacity) * 100))}%`,
                        }"
                      ></div>
                    </div>
                  </td>
        ...
        <!-- Clean Footer Link -->
        <div
          class="pt-3 mt-2 border-t dark:border-gray-700/60 flex items-center justify-between text-xs text-gray-400"
        >
          <span>{{
            isKindergarten ? "Sig'im va bolalar balansi" : "Sig'im va o'quvchilar balansi"
          }}</span>
          <RouterLink
            :to="isKindergarten ? '/students' : '/school/classes'"
      ...
      >
        <div>
          <!-- Subtitle Header -->
          <div
            class="pb-3 mb-3 border-b dark:border-gray-700/60 flex items-center justify-between text-xs"
          >
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
                <Icon icon="solar:user-plus-bold" />
              </div>
              <div class="min-w-0">
                <h4
                  class="font-bold text-xs text-gray-800 dark:text-gray-100 group-hover:text-primary transition truncate"
                >
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-xl bg-cyan-50 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 flex items-center justify-center text-lg shrink-0 group-hover:scale-105 transition"
              >
