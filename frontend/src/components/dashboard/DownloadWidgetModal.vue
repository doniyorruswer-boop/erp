  ...
  >
    ...
    <template #body>
      <div class="space-y-4 text-left">
        <!-- Widget Selection -->
        <div>
          <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2.5">
            Vidjetni tanlang
          </label>
    ...
    <template #footer>
      <div class="flex items-center justify-end gap-2.5 w-full">
        ...
        >
          <Icon v-if="downloading" icon="solar:refresh-circle-linear" class="animate-spin text-sm" />
...
export default {
  ...
  computed: {
    formatHint() {
      if (this.selectedFormat === "PNG") return "Yuqori aniqlikdagi tasvir (prezentatsiya va hisobotlar uchun)";
  ...
  methods: {
    ...
    async handleDownload() {
      ...
      try {
        ...
        const blob = new Blob([`EduHub CRM - ${widgetName} (${this.targetMonth})`], { type: "text/plain" });
