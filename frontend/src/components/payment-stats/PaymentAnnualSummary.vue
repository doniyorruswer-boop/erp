<template>
  <div
    class="p-4 sm:p-5 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-4"
  >
    <!-- Chap qism: Belgisi va Ta'rif -->
    <div class="flex items-center gap-3">
      <div
        class="w-10 h-10 rounded-xl bg-teal-500/10 flex items-center justify-center text-teal-500 text-xl shrink-0"
      >
      ...
      <div>
        ...
        <span
          :class="[
            ...
            summary.totalDebt > 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-400',
          ]"
        >
          {{ formatUZS(summary.totalDebt) }}
          <span v-if="summary.debtorsCount > 0" class="text-xs font-normal"
            >({{ summary.debtorsCount }} nafar)</span
          >
        </span>
      </div>
    </div>
