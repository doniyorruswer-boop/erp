    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left border-collapse min-w-[850px]">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60 text-xs sm:text-sm">
          ...
          >
            <td class="py-3 px-4 text-center text-gray-400 font-medium">{{ (currentPage - 1) * perPage + idx + 1 }}</td>
            ...
            <td class="py-3 px-4 font-bold" :class="c.debt > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-gray-400'">
              {{ formatUZS(c.debt) }}
            </td>
            <td class="py-3 px-4">
              <div class="space-y-1">
                <div class="flex justify-between text-xs font-semibold">
                  <span>{{ c.percent }}%</span>
                  <span :class="c.percent >= 80 ? 'text-emerald-600' : c.percent >= 50 ? 'text-amber-500' : 'text-rose-600'">
                ...
                <div class="w-full h-2 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
                  <div
                    ...
                    :class="c.percent >= 80 ? 'bg-emerald-500' : c.percent >= 50 ? 'bg-amber-500' : 'bg-rose-500'"
            ...
            <td class="py-3 px-4 text-right">
              <router-link :to="`/school/classes/${c.id}?tab=payments`">
                <AppButton size="sm" variant="outline" icon="solar:document-text-linear" title="Sinf to'lov tabelini ochish">
                  To'lov varaqasi
                </AppButton>
              </router-link>
            </td>
