...
type MockJwt = Record<string, jest.Mock>;
...
describe("AuthService (Unit Tests)", () => {
  ...
  describe("login", () => {
    ...
    it("should reject login if account is currently locked out", async () => {
      ...
      await expect(service.login({ phone: "+998901234567", password: "admin123" })).rejects.toThrow(
    ...
    it("should throw UnauthorizedException if user not found or inactive", async () => {
      ...
      await expect(service.login({ phone: "+998999999999", password: "admin" })).rejects.toThrow(
  ...
  describe("refreshTokens", () => {
    ...
    it("should throw ForbiddenException and invalidate token if hash does not match (reuse detected)", async () => {
      ...
      await expect(service.refreshTokens("tampered-or-reused-token")).rejects.toThrow(
        ForbiddenException
      );
      expect(prisma.user.update).toHaveBeenCalledWith({
