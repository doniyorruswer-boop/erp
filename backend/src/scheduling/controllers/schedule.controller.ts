...
export class ScheduleController {
  ...
  cancel(
    @Param("id") id: string,
    @Body() body: CancelScheduleDto,
    @CurrentTenant() orgId: string,
    @CurrentBranch() branchCtx: BranchContext,
    @Request() req: AuthenticatedRequest
  ) {
    return this.scheduleService.cancel(id, body, orgId, req.user?.id, branchCtx);
  }
  ...
  reschedule(
    ...
    return this.scheduleService.reschedule(id, body, orgId, req.user?.id, branchCtx);
  }
}
