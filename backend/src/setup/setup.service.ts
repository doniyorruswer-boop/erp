...
import { BusinessType, Role, AuditAction } from '@prisma/client';
...
export class SetupService {
  ...
  async initialize(data: any) {
    ...
    const defaultConfig = {
      ...
      terminology: data.terminology || {
        ...
        teacherLabel: data.businessType === BusinessType.SCHOOL ? "O'qituvchi" : "Mentor / O'qituvchi",
  ...
  async getModules(orgId?: string) {
    const allModules = [
      { id: 'STUDENTS', name: "O'quvchilar va Mijozlar", description: "O'quvchilar bazasi, shartnomalar, balans va ota-onalar", category: 'CORE' },
      { id: 'GROUPS', name: "Sinflar va Guruhlar", description: "O'quv guruhlari, xonalar, dars jadvali", category: 'CORE' },
      { id: 'COURSES', name: "Kurslar va Fanlar", description: "O'quv dasturlari va narxlar", category: 'CORE' },
      { id: 'ATTENDANCE', name: "Davomat va Baholash", description: "Dars davomati va imtihon jurnali", category: 'ACADEMIC' },
      { id: 'FINANCE', name: "Moliya va Xarajatlar", description: "Kassalar, xarajatlar, hisob-fakturalar", category: 'FINANCE' },
      { id: 'PAYMENTS', name: "To'lovlar va Kvitansiyalar", description: "To'lov qabuli, qaytarishlar, kassa operatsiyalari", category: 'FINANCE' },
      { id: 'LEADS', name: "Lidlar va CRM Voronka", description: "Yangi arizalar, kanban voronka, konversiya", category: 'CRM' },
      { id: 'SMS', name: "SMS va Bildirishnomalar", description: "Telegram, SMS, email xabarnomalar", category: 'COMMUNICATION' },
      { id: 'SCHEDULING', name: "Xonalar va Resurslar Taqsimoti", description: "Xonalar, proyektorlar, avto taqvim", category: 'OPERATIONS' },
      { id: 'WORKFLOW', name: "Workflow va Avtomatlashtirish", description: "Triggler va avtomatik qoidalar", category: 'AUTOMATION' },
    ];

    let enabledList = ['LEADS', 'STUDENTS', 'GROUPS', 'COURSES', 'ATTENDANCE', 'FINANCE', 'SMS', 'PAYMENTS'];
