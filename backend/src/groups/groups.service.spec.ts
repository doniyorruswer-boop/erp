...
describe('GroupsService (Unit Tests)', () => {
  ...
  describe('create', () => {
    it('should create a group successfully when valid course and room are provided', async () => {
      ...
      const mockRoom = { id: 'room-1', organizationId: 'org-1', branchId: 'branch-1', name: 'Room 101' };
  ...
  describe('generateLessonsForGroup', () => {
    it('should generate lessons for ODD_DAYS (Mon, Wed, Fri) between startDate and endDate', async () => {
      ...
      prisma.lesson.create.mockImplementation(({ data }: any) => Promise.resolve({ id: `l-${Math.random()}`, ...data }));
    ...
    it('should generate lessons for EVEN_DAYS (Tue, Thu, Sat)', async () => {
      ...
      const daysCreated = prisma.lesson.create.mock.calls.map((c: any) => new Date(c[0].data.date).getDay());
    ...
    it('should be idempotent: skip dates that already have a lesson', async () => {
      ...
      const mockGroup = {
        ...
        lessons: [
          { id: 'l-existing-1', date: existingDate, title: '1-dars' },
        ],
      };
      ...
      const callDates = prisma.lesson.create.mock.calls.map((c: any) => new Date(c[0].data.date).getDate());
    ...
    it('should throw NotFoundException if group does not exist', async () => {
      ...
      await expect(
        service.generateLessonsForGroup('non-existent', 'org-1'),
