...
export class GroupsService {
  ...
  async create(
    ...
    if (data.roomId) {
      ...
      if (!room)
        throw new BadRequestException("Xona topilmadi yoki ushbu tashkilotga tegishli emas");
  ...
  async update(
    ...
    let targetBranchId: string | null | undefined = group.branchId;
  ...
  async addStudent(
    ...
    if (group.branchId && student.branchId && group.branchId !== student.branchId) {
      throw new BadRequestException(
        "O'quvchi boshqa filialga tegishli. Guruh va o'quvchi bir xil filialga tegishli bo'lishi shart"
      );
    }
    ...
    if (group.roomId) {
      ...
      if (room && room.capacity > 0) {
        ...
        if (currentActiveCount >= room.capacity) {
          throw new BadRequestException(
            `Guruh xonasi sig'imi (${room.capacity} nafar) to'lgan. Yangi o'quvchi qo'shib bo'lmaydi`
          );
        }
      }
    }
  ...
  async transferStudent(
    ...
    if (!sourceGroup)
      throw new NotFoundException("Manba guruhi topilmadi yoki ushbu filialga kirish huquqi yo'q");
    ...
    if (!targetGroup)
      throw new NotFoundException("Maqsad guruhi topilmadi yoki ushbu filialga kirish huquqi yo'q");
    ...
    if (targetGroup.branchId && student.branchId && targetGroup.branchId !== student.branchId) {
      throw new BadRequestException(
        "Maqsad guruhi boshqa filialga tegishli. O'quvchini boshqa filialdagi guruhga ko'chirish taqiqlanadi"
      );
    }
    ...
    if (targetGroup.roomId) {
      ...
      if (room && room.capacity > 0) {
        ...
        if (currentActiveCount >= room.capacity) {
          throw new BadRequestException(
            `Maqsad guruhi xonasi sig'imi (${room.capacity} nafar) to'lgan. O'quvchini ko'chirib bo'lmaydi`
          );
        }
      }
    }
  ...
  async getTeacherWorkload(teacherId: string | undefined, orgId: string) {
    const teachers = await this.prisma.user.findMany({
      where: {
        ...
        ...(teacherId
          ? {}
          : { OR: [{ role: "TEACHER" }, { taughtGroups: { some: { deletedAt: null } } }] }),
        deletedAt: null,
      },
      select: {
        id: true,
  ...
  async generateLessonsForGroup(
    ...
    const targetLessonCount =
      options?.count && options.count > 0
        ...
        : group.course?.lessonCount && group.course.lessonCount > 0
          ...
          : group.course?.duration && group.course.duration > 0
            ? group.course.duration * 12
            : 12;
    ...
    if (candidateDates.length === 0) {
      return {
        message:
          "Yangi generatsiya qilinadigan darslar mavjud emas (barcha darslar allaqachon yaratilgan)",
        createdCount: 0,
