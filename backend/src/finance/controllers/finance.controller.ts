...
export class FinanceController {
  ...
  allocatePayment(@Body() body: AllocatePaymentDto, @CurrentTenant() orgId: string, @Request() req: any) {
    return this.financeService.allocatePayment(body, orgId, req.user?.id);
  }

  @Post('refund')
  @RequirePermissions('payments.refund')
  refundPayment(@Body() body: CreateRefundDto, @CurrentTenant() orgId: string, @Request() req: any) {
